...
!
 ...
 1    p(i)=pp(i)
      ...
      SUBROUTINE OPENOUT(OUT, STRNAME)
c Open a redirection of TAUOLA's print.
      COMMON / INOUT / INUT,IOUT
      INTEGER :: OUT
      ...
      IOUT = OUT
      IF ((IOUT.NE.0).AND.(IOUT.NE.6)) THEN
        INQUIRE(FILE=STRNAME, EXIST=EXIST)
        IF (EXIST) THEN
          OPEN(IOUT, FILE=STRNAME, STATUS='old', ACTION='write')
        ELSE
          OPEN(IOUT, FILE=STRNAME, STATUS='new', ACTION='write')
        ENDIF
      ENDIF
      END

      SUBROUTINE CLOSOUT()
c Close the redirection of TAUOLA's print.
      COMMON / INOUT / INUT,IOUT
      IF ((IOUT.NE.0).AND.(IOUT.NE.6)) THEN
        CLOSE(IOUT)
      ENDIF
